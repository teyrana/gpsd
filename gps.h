...
struct arpa_t
{
    int number;      /* track number of target */
	double range;    /* range in meters */
	double bearing;  /* bearing from sensor to target */
	int relative;   /* true  <= 'R': angles are relative to sensor's heading */
	                 /* false <= 'T': angles are relative to true north */
	char units;      /* Range Units:  K=Kilometers  */
	                 /*               N=Knots */
	                 /*               S=Statute miles */
	double speed;
	double course;
...
#define ARPA_SET        (1llu<<40)
#define SET_HIGH_BIT	41
...
extern void arpa_clear_report(struct arpa_t* arpa_p);
